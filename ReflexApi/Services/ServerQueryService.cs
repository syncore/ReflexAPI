namespace ReflexAPI.Services
{
    ...
    /// </summary>
    public class ServerQueryService : Service
    {
        private const int MaxHostsAllowed = 10;
        ...
        public object Any(ServerQueryRequest request)
        {
            LoggerUtil.LogInfoAndDebug(
                string.Format(
                    "Received manual user server query request for hostname(s): {0} and port(s): {1}",
        ...
        private bool HostsPortsValid(string[] hostnames, string[] ports)
        {
            ...
            if (!portsValid || !hostsValid)
            {
                LoggerUtil.LogInfoAndDebug(
                    "Failed manual user server query request: user's multi-server query contained invalid hostname and/or port",
                    LogClassType);
            }
        ...
        private bool HostsPortsValid(string hostname, string port)
        {
            ...
            {
                LoggerUtil.LogInfoAndDebug(
                    "Failed manual user server query request: user's single-server query contained invalid hostname and/or port",
                    LogClassType);
            }
        ...
        /// <returns>
        /// <c>true</c> if the master server list contains the specified ip, otherwise <c>false</c>.
        /// </returns>
        private bool MasterServerListContainsHost(IPAddress ip)
        {
            return ServerList.AllServers.Any(s => s.ip.Equals(ip.ToString()));
        }
        ...
        /// <returns>
        /// A <see cref="ServerQueryResponse"/> to the user that includes an error message.
        /// </returns>
